...
function mostSpokenLanguages(countries, n) {
  let mostSpokenCountries = [];

  let allCountriesList = countries.reduce(
    (acc, item) => [...acc, ...item.languages],
    []
  );
  let allUniqueCountries = new Set(allCountriesList);
  //console.log(allUniqueCountries)

  let countriesMap = [];

  for (let country of allUniqueCountries) {
    // console.log(country)
    countriesMap.push([
      country,
      allCountriesList.filter((c) => c == country).length,
    ]);
  }

  return new Map(countriesMap.sort((a, b) => b[1] - a[1]).slice(0, n));
}

console.log(mostSpokenLanguages(newCountries, 10));
